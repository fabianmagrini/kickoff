...
│   │   ├── competitions/      ← repository + server fn + query options + tests
...
│   │   ├── leagues/           ← repository + server fn + competition-scoped
...
│   │   ├── index.tsx          ← competition selector (redirects to active competition)
│   │   ├── competitions/      ← all competitions + per-competition dashboard, fixtures, leaderboard, leagues
│   │   ├── matches/           ← match detail only ($matchId — globally unique, no competition nesting)
│   │   ├── admin.tsx          ← admin UI with competition selector
...
__root (layout: Navbar + QueryClientProvider)
  ├── /                                     (competition selector → redirect to active competition)
  ├── /login
  ├── /competitions/                        (all competitions list)
  │   └── /competitions/$competitionId/     (competition dashboard)
  │       ├── /matches/                     (fixture list, scoped to competition)
  │       ├── /leaderboard                  (per-competition leaderboard)
  │       └── /leagues/                     (leagues list + create)
  │           ├── /join                     (join by invite code)
  │           └── /$leagueId                (league detail + scoped leaderboard)
  ├── /matches/$matchId                     (match detail — global, match ID is UUID-unique)
  ├── /profile
  ├── /admin
  └── /api/
