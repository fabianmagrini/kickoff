...
/** Navigate to / and extract the competition ID from the redirect URL. */
...
test('navbar has Competitions link', async ({ page }) => {
  await page.goto('/');
  const nav = page.getByRole('navigation');
  await expect(nav.getByRole('link', { name: 'Competitions' })).toBeVisible();
});
...
test('authenticated user sees personalised welcome heading and stats cards', async ({ page }) => {
  ...
  // After sign-up, / redirects to competition dashboard
  await page.waitForURL(/\/competitions\/.+/, { timeout: 10_000 });
