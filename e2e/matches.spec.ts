...
test('unauthenticated user sees sign-in prompt on match detail', async ({ page }) => {
  ...
  // Matches are links containing "vs" in the fixture list.
  // This test requires at least one seeded match (run: npm run db:seed:dev).
  const firstMatch = page.getByRole('link', { name: /vs/i }).first();
  const matchCount = await firstMatch.count();
  test.skip(matchCount === 0, 'No matches seeded — run npm run db:seed:dev');
